...
class _HomeState extends State<Home> with TickerProviderStateMixin<Home> {
  ...
  void initState() {
    super.initState();
    // make navigator keys for each destination
    navigatorKeys = List<GlobalKey<NavigatorState>>.generate(
      allDestinations.length,
      (int index) => GlobalKey(),
    ).toList();

    // make fade controllers for each destination
          ...
          child: Stack(
            ...
            children: allDestinations.map((Destination destination) {
              ...
              if (index == selectedIndex) {
                destinationFaders[index].forward(); // show selected destination
                return Offstage(offstage: false, child: view);
              } else {
                destinationFaders[index].reverse(); // hide unselected destinations
